...
st.set_page_config(page_title="Interview | Gatsby AI Interview",
                   page_icon=config.FAVICON_PATH)

# Create columns in the sidebar to center a smaller image
col1, col2, col3 = st.sidebar.columns([1, 2, 1])
with col2:
    # Display smaller centered image without pixelation
    # by retaining aspect ratio
...
with col2:
    ...
    if st.session_state.interview_active and st.button(
        ...
        timestamped_username = (
            ...
            f"{st.session_state.start_time_file_names}"
        )
            ...
            with st.spinner(
                ...
                if test_connection():
                    backup_file = os.path.join(
                        ...
                        f"{timestamped_username}.json"
                    )
                    if os.path.exists(backup_file):
                        os.remove(backup_file)
                        st.sidebar.info(
                            "Backup deleted after successful MongoDB save.")
...
if st.session_state.user_info_submitted:
    for message in st.session_state.messages[1:]:
        ...
        if not any(code in message["content"]
...
# In case the interview history is still empty,
# pass system prompt to model, and
# generate and display its first message
# but only if user info has been submitted
    ...
    if api == "openai":
        ...
        with st.chat_message("assistant", avatar=config.AVATAR_INTERVIEWER):
            try:
                ...
                # This will be logged in the console only
                print(f"OpenAI API error: {str(e)}")
                message_interviewer = ("I apologize, but we're having trouble "
        ...
        with st.chat_message("assistant", avatar=config.AVATAR_INTERVIEWER):
            ...
            try:
                ...
                message_interviewer = (
                    "I apologize, but we're having trouble "
                    "connecting right now. Please try again later."
                )
                message_placeholder.markdown(message_interviewer)
...
if (
    ...
    and not st.session_state.user_info_submitted
):
    st.markdown(
...
# Main chat if interview is active and user info has been submitted
    ...
    st.session_state.interview_active
    and st.session_state.user_info_submitted
):
        ...
        with st.chat_message("assistant", avatar=config.AVATAR_INTERVIEWER):
            ...
            if api == "openai":
                ...
                try:
                    ...
                    for message in stream:
                        ...
                        if text_delta is not None:
                            message_interviewer += text_delta
                        # Start displaying message only after
                        # 5 characters to first check for codes
                        ...
                        if any(
                            ...
                            # of the message in case of a code
                    ...
                    st.info(
                        "Please inform the study coordinator about this issue"
                    )
                    # Log the error without displaying traceback in UI
                    # This will be logged in the console only
                    print(f"OpenAI API error: {str(e)}")
                    message_interviewer = (
                        "I apologize, but we're having trouble connecting "
            ...
            if not any(
                ...
                try:
                    ...
                    document = prepare_mongo_data(
                        username=(
                            f"{st.session_state.username}_backup_"
                            f"{st.session_state.start_time_file_names}"
                        ),
                        transcript=transcript,
                        time_data=time_data,
            ...
            # If code in the message, display the associated closing message
            # Loop over all codes
            for code in config.CLOSING_MESSAGES.keys():
                        ...
                        with st.spinner(
                            ...
                            if success:
                                ...
                                st.error(
                                    ...
                                    "temporary backup saved locally"
                                )
                    except Exception as e:
                        st.sidebar.error(f"Failed to save to MongoDB: {e}")
