                    ...
                    with st.expander(f"## Interview with {username}", expanded=True):
                        ...
                        with st.container():
                            ...
                            if sentiments and isinstance(sentiments, dict):
                                title = render_analysis_date(interview.get("analyzed_at"), "Sentiment Analysis")
                                st.markdown(title)
                            ...
                            if not isAnalysed:
                                with col1:
                                    st.button("Analyse", key=f"analyse-{interview.get('_id')}", 
                                            on_click=reanalyse_and_refresh, args=(interview.get('_id'), "Student"),
                                            use_container_width=True)
                            with col2:
                                st.button("Delete", key=f"delete-{interview.get('_id')}", 
                                        on_click=delete_and_refresh, args=(interview.get('_id'), "Student"),
                                        use_container_width=True)
            else:
                    ...
                    with st.expander(f"## Interview with {username}", expanded=True):
                        ...
                        with cols[1]:
                            ...
                            if not isAnalysed:
                                with col1:
                                    st.button("Analyse", key=f"analyse-{interview.get('_id')}", 
                                            on_click=reanalyse_and_refresh, args=(interview.get('_id'), "Staff"),
                                            use_container_width=True)
                            with col2:
                                st.button("Delete", key=f"delete-{interview.get('_id')}", 
                                        on_click=delete_and_refresh, args=(interview.get('_id'), "Staff"),
                                        use_container_width=True)
            else:
