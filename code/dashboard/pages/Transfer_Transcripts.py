...
def extract_documents(db, collection_name, query):
    try:
        ...
        if not documents:
            st.info(
                f"No documents found in {collection_name} matching the query.")
            return None, 0
...
def insert_documents(db, dest_name, documents):
    try:
        ...
        for i, document in enumerate(documents):
            ...
            status_text.text(
                f"Inserted {i+1}/{doc_count} documents ({progress}%)")
...
with col2:
    destination_collection = st.text_input(
        "Destination Collection", value="students")
...
with date_col1:
    start_date = st.date_input(
        "Start Date", value=datetime.now() - timedelta(days=1))
    start_time = st.time_input(
        "Start Time", value=datetime.strptime("00:00:00", "%H:%M:%S").time())

with date_col2:
    end_date = st.date_input("End Date", value=datetime.now())
    end_time = st.time_input(
        "End Time", value=datetime.strptime("23:59:59", "%H:%M:%S").time())
...
if st.button("Extract Documents for Review"):
    ...
    if client is not None and db is not None:
        ...
        if docs:
            ...
            st.session_state.selected_docs = {
                str(doc.get('_id', i)): True for i, doc in enumerate(docs)}
            st.session_state.select_all = True
...
if st.session_state.extraction_complete:
    st.subheader(
        f"Review Extracted Documents ({st.session_state.extraction_count})")

    # Function to handle individual checkbox changes
    def on_checkbox_change(doc_id):
        # Update the selected_docs dictionary
        st.session_state.selected_docs[doc_id] = st.session_state[
            f"doc_{doc_id}"
        ]
    ...
    def toggle_all():
        ...
        for doc_id in st.session_state.selected_docs:
            ...
            # Update the checkbox widget state
            st.session_state[f"doc_{doc_id}"] = new_state
    ...
    # Add details expander for full document inspection
    # only show selected documents
    with st.expander("View Selected Documents Details"):
        ...
        selected_docs_list = [
            ...
            if st.session_state.selected_docs.get(
                str(doc.get('_id', i)),
                False
            )
        ]
        ...
        if not selected_docs_list:
            ...
            st.write(
                f"Showing details for {len(selected_docs_list)} "
                "selected document(s):"
            )
            ...
            if len(selected_docs_list) > 1:
                tabs = st.tabs(
                    [f"Doc {i+1}" for i in range(len(selected_docs_list))])
    ...
    selected_count = sum(
        1 for selected in st.session_state.selected_docs.values() if selected)
    ...
    if st.button(f"Insert Selected Documents ({selected_count})"):
        ...
        if client is not None and db is not None:
            ...
            if not selected_docs:
                ...
                if inserted > 0:
                    st.success(
                        f"Successfully inserted {inserted} documents "
                        f"into '{destination_collection}'")
